...
def bootstrap_filesystem():
    ...
    filesystem.mount(bootstrap.ENGINE_MOUNT, fsp.FileSystemProvider(ENGINE_PATH))
    filesystem.mount(bootstrap.MODULE_MOUNT, fsp.FileSystemProvider(MODULE_PATH))
    filesystem.mount(bootstrap.COMMON_MOUNT, fsp.FileSystemProvider(COMMON_PATH))

def bootstrap_game_filesystem(base):
    """ Bootstrap the game. """
    import rave.modules.filesystemprovider as fsp
